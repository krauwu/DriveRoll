...
def downsample_clr_blockwise(img_u8, target_size):
    """
    img_u8: (H,W,3) uint8
    target_size: (H2,W2)
    """
    ...
    valid = (img_u8.sum(axis=2) > 0).astype(np.float32)  # (H,W) 1=valid,0=invalid

    # 加权累加（无效像素权重=0）
    num = cv2.resize(img * valid[..., None], (W2, H2), interpolation=cv2.INTER_AREA)
    den = cv2.resize(valid, (W2, H2), interpolation=cv2.INTER_AREA)  # (H2,W2)
