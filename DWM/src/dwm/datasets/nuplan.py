...
from dwm.fs.obsfs import DirFileSystem
...
class NuPlanDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.fs = fs if fs is not None else DirFileSystem(path=sensor_root)
    ...
    def _get_sensor_paths(self, info):
        ...
        for ch in self.sensor_channels:
            ...
            if isinstance(p, str) and (p.startswith("obs://") or os.path.isabs(p)):
                paths.append(p)
                continue

            paths.append(os.path.join(self.sensor_root, p))
    ...
    def _compose_points_for_frame(self, info):
        ...
        min_actor_pts = int(s.get("min_actor_points", 100000))
        ...
        for i, box in enumerate(boxes):
            ...
            # --- load track (always 6 cols xyzrgb) ---
            actor_track = None
            if tok is not None and self._actor_root is not None:
                p = os.path.join(self._actor_root, f"{tok}.npy")
                if os.path.isfile(p):
                    tmp = np.asarray(np.load(p))
                    if tmp.shape[0] > 0:
                        actor_track = tmp

            # --- pick template (fallback geometry for depth/sem) ---
            if cls == "car":
                actor_tpl = self._actor_tpl.get("suv")
            elif cls == "ped":
                actor_tpl = self._actor_tpl.get("ped")
            elif cls == "bike":
                actor_tpl = self._actor_tpl.get("bike")
            else:
                actor_tpl = None
            if actor_tpl is None:
                continue

            actor_tpl = np.asarray(actor_tpl)
            tpl_xyz = actor_tpl[:, :3].astype(np.float32)

            Rz = _yaw_to_Rz(yaw)
            t_xyz = np.array([x, y, z], np.float32)[None, :]

            # ============================================================
            # A) depth/sem xyz: template by default;
            #    only if track points >= min_actor_pts -> use track xyz
            # ============================================================
            use_track_for_depth = False
            if actor_track is not None:
                # actor_track is 6 cols, but keep it safe
                if actor_track.shape[1] >= 3 and actor_track.shape[0] >= min_actor_pts:
                    use_track_for_depth = True

            if use_track_for_depth:
                xyz_sem = actor_track[:, :3].astype(np.float32)
            else:
                xyz_sem = tpl_xyz

            xyz_sem = (Rz @ xyz_sem.T).T + t_xyz
            act_xyz_list.append(xyz_sem)
            act_sem_list.append(np.full((xyz_sem.shape[0],), sid, np.int32))

            # ============================================================
            # B) clr xyz/rgb: ALWAYS use track; if missing -> skip this actor clr
            # ============================================================
            if actor_track is None:
                continue
            if actor_track.shape[1] < 6:
                continue  # 理论上不会发生，但防御一下

            xyz_clr = actor_track[:, :3].astype(np.float32)
            xyz_clr = (Rz @ xyz_clr.T).T + t_xyz
    ...
    def __getitem__(self, index):
        ...
        for x in seq:
            ...
            for cam_i, (cam_ch, p) in enumerate(zip(self.sensor_channels, paths)):
                if not p:
                    raise ValueError(
                        f"Empty image path at scene={scene}, cam={cam_ch}, cam_i={cam_i}, index={index}"
                    )
                ...
                if cam_info is None:
                    raise ValueError(
                        f"Missing cam_info at scene={scene}, cam={cam_ch}, cam_i={cam_i}, path={p}"
                    )
                    ...
                    if need_depth or need_sem or need_clr:
                        ...
                        try:
                            ...
                            if need_depth and p_d and self.do_cache and os.path.isfile(p_d):
                                bins_u16 = _try_open_u16_png(p_d) or bins_u16
