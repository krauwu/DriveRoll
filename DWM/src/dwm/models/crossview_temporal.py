...
class RefCrossAttentionBlock(torch.nn.Module):
    """
      norm_in  -> FF_in          -> 残差
      norm_x   -> Cross-Attn(ref)-> 残差
      norm_out -> FF             -> 残差
    暂时未加入 pe
    输入张量支持 [B, HW, C] 或 [B, C, H, W]（diffusers.Attention 自带兼容）。
    """
    def __init__(
        self,
        dim: int,
        ...
        enable_ref=False,
        enable_bev=False,
        add_zero_proj_ref: bool = True,
        add_zero_proj_bev: bool = True,
    ):
        super().__init__()
        self.is_res = dim == time_mix_inner_dim
        self.norm_in = torch.nn.LayerNorm(dim)
        ...
        if self.enable_ref:
            self.norm_x = torch.nn.LayerNorm(time_mix_inner_dim)
            self.cross_attn = diffusers.models.attention_processor.Attention(
                query_dim=time_mix_inner_dim,
                heads=num_attention_heads,
                dim_head=attention_head_dim,
                cross_attention_dim=time_mix_inner_dim,  # ref_feat 的最后维度需与此相同
                qk_norm=qk_norm
            )
            self.proj_ref = (
                torch.nn.Linear(time_mix_inner_dim, time_mix_inner_dim, bias=False)
                if add_zero_proj_ref else None
            )
            if self.proj_ref is not None:
                torch.nn.init.zeros_(self.proj_ref.weight)
                
        if self.enable_bev:
            self.norm_bev = torch.nn.LayerNorm(time_mix_inner_dim)
            self.cross_attn_bev = diffusers.models.attention_processor.Attention(
                query_dim=time_mix_inner_dim,
                heads=num_attention_heads,
                dim_head=attention_head_dim,
                cross_attention_dim=time_mix_inner_dim,  # bev_feat 通道也对齐
                qk_norm=qk_norm
            )
            self.proj_bev = (
                torch.nn.Linear(time_mix_inner_dim, time_mix_inner_dim, bias=False)
                if add_zero_proj_bev else None
            )
            if self.proj_bev is not None:
                torch.nn.init.zeros_(self.proj_bev.weight)


        self.norm_out = torch.nn.LayerNorm(time_mix_inner_dim)
        self.ff = diffusers.models.attention.FeedForward(
            time_mix_inner_dim, activation_fn="geglu"
        )
    ...
    def forward(
        ...
        hidden_states = self.norm_in(hidden_states)
        hidden_states = self.ff_in(hidden_states)
        hidden_states = hidden_states + residual  # [B, HW, C] or [B, C, H, W]

        #cross-ref
        if self.enable_ref:
            enc_kv = torch.cat([ref_feat_front, ref_feat_rear], dim=1)  # [B, S_f+S_r, C]
        
            residual = hidden_states
            norm_hidden_states = self.norm_x(hidden_states)
            ref_out = self.cross_attn(
                norm_hidden_states,
                encoder_hidden_states=enc_kv,
            )
            if hasattr(self, "proj_ref") and self.proj_ref is not None:
                ref_out = self.proj_ref(ref_out)   #zero_init
                
            hidden_states = ref_out + residual
        
        if self.enable_bev:
            residual = hidden_states
            bevnorm_hidden_states = self.norm_bev(hidden_states)
            bev_out = self.cross_attn_bev(
                bevnorm_hidden_states, encoder_hidden_states=bev_feat  
            )
            if hasattr(self, "proj_bev") and self.proj_bev is not None:
                bev_out = self.proj_bev(bev_out)    # zero_init
            
            hidden_states = bev_out + residual

        residual = hidden_states
        norm_hidden_states = self.norm_out(hidden_states)
        ff_out = self.ff(norm_hidden_states)
        hidden_states = ff_out + residual
