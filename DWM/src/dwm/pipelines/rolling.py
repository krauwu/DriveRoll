...
class RollingForcing:
    ...
    def flatten_clip_text(
        ...
        if do_classifier_free_guidance:
            if isinstance(clip_text, str):
                ...
                for i in clip_text:
                    RollingForcing.flatten_clip_text(
    ...
    def get_action_ids(
        ...
        if streaming_mode:
            ego_transforms = torch.cat([
                batch["ego_transforms"] if prev_ego_transforms is None else prev_ego_transforms,
        ...
        if action_condition_mask is None:
            is_conditioned = (current_pose - uncondition_pose).sum((1, 2, 3, 4)).abs() > 1e-3
        else:
            is_conditioned = torch.logical_and(
                (current_pose - uncondition_pose).sum((1, 2, 3, 4)).abs() > 1e-3,
        ...
        speed = 3.6 * moving_distance * batch["fps"].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def get_conditions(
        ...
        if text_encoder is not None:
            ...
            if isinstance(model, diffusers.UNetSpatioTemporalConditionModel):
                ...
                if len(parsed_shape) == 1:
                    text_embeddings = text_embeddings.unsqueeze(1).unsqueeze(1).repeat(
                        1, sequence_length, view_count, 1, 1
                    )
                else:
                    text_embeddings = text_embeddings.unflatten(0, parsed_shape)
                ...
                for clip_tokenizer, clip_text_encoder in zip(tokenizer[:2], text_encoder[:2]):
                    text_embeddings, pooled_text_embeddings = \
                        RollingForcing.sd3_encode_prompt_with_clip(
                    ...
                    clip_pooled_text_embeddings_list.append(pooled_text_embeddings)

                clip_text_embeddings = torch.cat(clip_text_embeddings_list, dim=-1)
                pooled_text_embeddings = torch.cat(clip_pooled_text_embeddings_list, dim=-1)

                t5_prompt_embed = RollingForcing.sd3_encode_prompt_with_t5(
                ...
                clip_text_embeddings = torch.nn.functional.pad(
                    ...
                    (0, t5_prompt_embed.shape[-1] - clip_text_embeddings.shape[-1]),
                )
                text_embeddings = torch.cat([clip_text_embeddings, t5_prompt_embed], dim=-2)

                if len(parsed_shape) == 1:
                    text_embeddings = text_embeddings.unsqueeze(1).unsqueeze(1).repeat(
                        1, sequence_length, view_count, 1, 1
                    ).to(dtype=dtype)
                    pooled_text_embeddings = pooled_text_embeddings.unsqueeze(1).unsqueeze(1).repeat(
                        1, sequence_length, view_count, 1
                    ).to(dtype=dtype)
                else:
                    text_embeddings = text_embeddings.unflatten(0, parsed_shape).to(dtype=dtype)
                    pooled_text_embeddings = pooled_text_embeddings.unflatten(0, parsed_shape).to(dtype=dtype)

                condition_embedding_list.append(text_embeddings)

        condition_on_all_frames = common_config.get("condition_on_all_frames", False)
        uncondition_image_color = common_config.get("uncondition_image_color", 0)

        if "3dbox_images" in batch:
            _3dbox_images = (
                batch["3dbox_images"].to(device)
                if condition_on_all_frames else batch["3dbox_images"][:, :1].to(device)
            )
            if _3dbox_condition_mask is not None:
                _3dbox_images[_3dbox_condition_mask.logical_not().to(device)] = uncondition_image_color
            if do_classifier_free_guidance:
        ...
        if "hdmap_images" in batch:
            hdmap_images = (
                batch["hdmap_images"].to(device)
                if condition_on_all_frames else batch["hdmap_images"][:, :1].to(device)
            )
            if hdmap_condition_mask is not None:
                hdmap_images[hdmap_condition_mask.logical_not().to(device)] = uncondition_image_color
            if do_classifier_free_guidance:
        ...
        encoder_hidden_states = (
            torch.cat(condition_embedding_list, -2).to(dtype=dtype)
            if len(condition_embedding_list) > 0 else None
        )
        condition_image_tensor = (
            torch.cat(condition_image_list, -3)
            if len(condition_image_list) > 0 else None
        )

        if "added_time_ids" in common_config:
            if common_config["added_time_ids"] == "fps_camera_transforms":
                added_time_ids = torch.cat([
                    batch["fps"].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(
                        1, sequence_length, view_count, 1
                    ),
                    RollingForcing.get_camera_transform_ids(batch, common_config)
                ], -1)
                if do_classifier_free_guidance:
                    added_time_ids = torch.cat([added_time_ids, added_time_ids], 0)
                added_time_ids = added_time_ids.to(device)

            elif common_config["added_time_ids"] == "fps_camera_transforms_action":
                added_time_ids = torch.cat([
                    batch["fps"].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(
                        1, sequence_length, view_count, 1
                    ),
                    RollingForcing.get_camera_transform_ids(batch, common_config),
                    RollingForcing.get_action_ids(
                ...
                if do_classifier_free_guidance:
                    ...
                    added_time_ids = torch.cat([uncond_added_time_ids, added_time_ids], 0)
                added_time_ids = added_time_ids.to(device)
            else:
                added_time_ids = None
        ...
        if common_config.get("explicit_view_modeling", False):
            if "ego_transforms" not in batch:
                ...
                ego_transforms = batch["ego_transforms"][:, :, -batch["camera_transforms"].shape[2]:, ...]
            ...
            camera_intrinsics_norm[..., 0, 0] = camera_intrinsics_norm[..., 0, 0] / batch["image_size"][..., 0]
            camera_intrinsics_norm[..., 1, 1] = camera_intrinsics_norm[..., 1, 1] / batch["image_size"][..., 1]
            camera_intrinsics_norm[..., 0, 2] = camera_intrinsics_norm[..., 0, 2] / batch["image_size"][..., 0]
            camera_intrinsics_norm[..., 1, 2] = camera_intrinsics_norm[..., 1, 2] / batch["image_size"][..., 1]

            if "is_uncalibrated" in batch:
                camera_intrinsics_norm[batch["is_uncalibrated"], :, :] = torch.eye(3).to(batch["camera_transforms"])
                camera2referego[batch["is_uncalibrated"], :, :] = torch.eye(4).to(batch["camera_transforms"])
            if explicit_view_modeling_mask is not None:
                camera_intrinsics_norm[explicit_view_modeling_mask.logical_not().to(device)] = \
                    torch.eye(3).to(batch["camera_transforms"])
                camera2referego[explicit_view_modeling_mask.logical_not().to(device)] = \
                    torch.eye(4).to(batch["camera_transforms"])

            if do_classifier_free_guidance:
                camera_intrinsics_norm = torch.cat([camera_intrinsics_norm, camera_intrinsics_norm], 0)
                camera2referego = torch.cat([camera2referego, camera2referego], 0)

            camera_intrinsics_norm = camera_intrinsics_norm.to(device)
            camera2referego = camera2referego.to(device)

        has_depth_input = "camera_intrinsics" in batch and "camera_transforms" in batch
        if has_depth_input:
            camera_intrinsics = batch["camera_intrinsics"].to(device)
            camera_transforms = batch["camera_transforms"].to(device)
            if do_classifier_free_guidance:
                camera_intrinsics = torch.cat([camera_intrinsics, camera_intrinsics])
                camera_transforms = torch.cat([camera_transforms, camera_transforms])

        result = {
            "encoder_hidden_states": encoder_hidden_states,
            "condition_image_tensor": condition_image_tensor,
            "disable_crossview": torch.tensor(
                [common_config.get("disable_crossview", False)], device=device
            ).repeat(batch_size),
            "disable_temporal": torch.tensor(
                [common_config.get("disable_temporal", False)], device=device
            ).repeat(batch_size),
            ...
            ).to(device) if "crossview_mask" in batch else None,
            "camera_intrinsics": camera_intrinsics if has_depth_input else None,
            "camera_transforms": camera_transforms if has_depth_input else None,
            ...
            if common_config.get("explicit_view_modeling", False) else None,
            "added_time_ids": added_time_ids if "added_time_ids" in common_config else None
        }

        if isinstance(model, diffusers.SD3Transformer2DModel) and text_encoder is not None:
        ...
        if latents_shape is not None and latents_shape[1] != sequence_length:
            ...
            for k in result:
                if result[k] is not None and result[k].ndim > 1 and result[k].shape[1] == sequence_length:
                    result[k] = torch.cat([result[k][:, :pre], result[k][:, pre::stride]], dim=1)
    ...
    def make_depth_loss(
        ...
        frustum_from_lidar = (normalized_intrinsics @ camera_from_lidar).to(device)
        ...
        for i in range(batch_size):
            for j in range(sequence_length):
                points = dwm.functional.make_homogeneous_vector(
                    batch["lidar_points"][i][j][:, :3].to(device)
                ).t()
                projected_points = frustum_from_lidar[i, j] @ points
                p = (projected_points[:, 0:2] / projected_points[:, 2:3]).transpose(-2, -1)
                fd = ((projected_points[:, 2] - depth_frustum_range[0]) / depth_frustum_range[2]).round().long()
                ...
                for k in range(view_count):
                    ...
                    valid_sample_list.append(
                        torch.arange(0, point_count_limit_per_view, device=p.device) < count
                    )
                    if count > 0:
                        k_p = p[k, view_mask]
                        k_fd = fd[k, view_mask]
                        if count > point_count_limit_per_view:
                            ...
                            k_p = torch.nn.functional.pad(
                                k_p, (0, 0, 0, point_count_limit_per_view - count)
                            )
                        ...
                        k_p = torch.zeros(
                            (point_count_limit_per_view, 2),
                            dtype=p.dtype, device=p.device
                        )
                        k_fd = torch.zeros(
                            (point_count_limit_per_view),
                            dtype=fd.dtype, device=fd.device
                        )
                    p_list.append(k_p)
                    fd_list.append(k_fd)
        ...
        ground_truth_depth = torch.nn.functional.one_hot(
            torch.stack(fd_list), predicted_depth.shape[-3]
        ).transpose(-2, -1).float()
        valid_sample = torch.stack(valid_sample_list)

        depth_cross_entropy = torch.nn.functional.cross_entropy(
            depth_samples.flatten(0, 2), ground_truth_depth, reduction="none"
        )
        depth_estimation_loss = (depth_cross_entropy * valid_sample.float()).sum() / valid_sample.sum()
    ...
    def enum_depth_preds_and_targets(
        ...
        for i in range(batch_size):
            ...
            for j in range(sequence_length):
                points = dwm.functional.make_homogeneous_vector(
                    batch["lidar_points"][i][j].to(device)
                ).t()
                projected_points = frustum_from_lidar[i, j] @ points
                p = (projected_points[:, 0:2] / projected_points[:, 2:3]).transpose(-2, -1)
                fd = (projected_points[:, 2] - depth_frustum_range[0]) / depth_frustum_range[2]
                mask = torch.logical_and(
                    p.abs().amax(dim=-1) < 1,
                    torch.logical_and(fd >= -0.5, fd < depth_features.shape[-3] - 0.5)
                )
            ...
            for j in range(sequence_length):
                for k in range(view_count):
                    ...
                    if valid_count > 0:
                        pred_depth = depth_samples[j, k, 0, :valid_count] * depth_frustum_range[2]
                        target_depth = i_fd_list[j * view_count + k] * depth_frustum_range[2]
    ...
    def try_make_input_for_prediction(
        ...
        return made_noisy_input, timesteps, additional_condition, reference_frame_indicator
    ...
    def sd3_encode_prompt_with_clip(
        ...
        text_embeddings = text_embeddings.to(dtype=text_encoder.dtype, device=device)
    ...
    @staticmethod
    def sd3_get_sigmas(noise_scheduler, timestep_indices, n_dim, device, dtype):
        sigmas = noise_scheduler.sigmas[timestep_indices].to(device=device, dtype=dtype)
    ...
    def __init__(
        ...
        self.should_save = not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0
        self.config = config
        self.device = device
        ...
        distribution_framework = self.common_config.get("distribution_framework", "ddp")
        if not torch.distributed.is_initialized() or distribution_framework == "ddp":
            self.model.to(self.device)
        elif distribution_framework == "fsdp" and "fsdp_ignored_module_pattern" in common_config:
        ...
        if isinstance(self.model, diffusers.UNetSpatioTemporalConditionModel):
            ...
            if torch.distributed.is_initialized() and distribution_framework == "fsdp":
        ...
        if isinstance(self.model, diffusers.UNetSpatioTemporalConditionModel):
            ...
            self.train_scheduler = diffusers.FlowMatchEulerDiscreteScheduler.from_pretrained(
                pretrained_model_name_or_path, subfolder="scheduler"
            )
            test_scheduler_type = dwm.common.get_class(
        ...
        if resume_from is not None:
            state_dict = RollingForcing.load_state(
                os.path.join(output_path, "checkpoints", "{}.pth".format(resume_from))
            )
            self.model.load_state_dict(state_dict)
        elif model_checkpoint_path is not None:
            state_dict = RollingForcing.load_state(model_checkpoint_path)
            if isinstance(self.model, diffusers.UNetSpatioTemporalConditionModel):
                state_dict = dwm.models.crossview_temporal_unet.UNetCrossviewTemporalConditionModel \
        ...
        if self.should_save:
            param_count = sum(i.numel() for i in self.model.parameters() if i.requires_grad)
        ...
        if self.training_config.get("enable_grad_scaler", False):
            if not torch.distributed.is_initialized() or self.distribution_framework == "ddp":
                self.grad_scaler = torch.amp.GradScaler()
            elif self.distribution_framework == "fsdp":
                self.grad_scaler = torch.distributed.fsdp.sharded_grad_scaler.ShardedGradScaler()
            ...
            if self.distribution_framework == "ddp":
                ...
                if "fsdp_ignored_module_pattern" in common_config:
                    ...
                    if self.should_save:
                        ...
                        print("These ignored modules are {}.".format([i[0] for i in ignored_named_modules]))
                else:
                ...
                raise Exception(
                    "Unknown data parallel framework {}.".format(self.distribution_framework)
                )
    ...
    def save_checkpoint(self, output_path: str, steps: int):
        if torch.distributed.is_initialized():
            ...
            model_state_dict = torch.distributed.checkpoint.state_dict.get_model_state_dict(
                self.model_wrapper, options=options
            )
        ...
        if self.should_save:
            torch.save(
                model_state_dict,
                os.path.join(output_path, "checkpoints", "{}.pth".format(steps))
            )
    ...
    def log(self, global_step: int, log_steps: int):
        if self.should_save:
            if len(self.loss_report_list) > 0 and isinstance(self.loss_report_list[0], dict):
                loss_values = {
                    i: sum(j[i] for j in self.loss_report_list) / len(self.loss_report_list)
                ...
                print(
                    "Step {} ({:.1f} s/step), {}".format(
                        global_step, self.step_duration / log_steps, loss_message
                    )
                )
                for key, value in loss_values.items():
                    self.summary.add_scalar("train/{}".format(key), value, global_step)
            else:
    ...
    def train_step(self, batch: dict, global_step: int):
        ...
        noise = torch.randn(latents.shape, generator=self.generator).to(self.device)
        ...
        if isinstance(self.model, diffusers.UNetSpatioTemporalConditionModel):
            ...
            u = RollingForcing.sd3_compute_density_for_timestep_sampling(
            ...
            timestep_indices = (u * self.train_scheduler.config.num_train_timesteps).long()
            timesteps = self.train_scheduler.timesteps[timestep_indices].to(self.device)

            sigmas = RollingForcing.sd3_get_sigmas(
        ...
        while len(timesteps.shape) < 3:
            timesteps = timesteps.unsqueeze(-1).repeat_interleave(
                latents.shape[len(timesteps.shape)], -1
            )
        ...
        with self.get_autocast_context():
            model_conditions = RollingForcing.get_conditions(
                self.model,
            ...
            if self.training_config.get("extra_reference_infer", 0) > 0:
                extra_image_tensor = image_tensor[:, :, :self.training_config.get("extra_reference_infer", 0)]
            ...
            noisy_latents, timesteps, additional_conditions, reference_frame_indicator = \
                RollingForcing.try_make_input_for_prediction(
            ...
            sd_pred_latent = (
                sd_pred[0] * (-sigmas) + noisy_latents
        ...
        if len(sd_pred) > 1:
            ...
            loss_dict["depth_loss"] = RollingForcing.make_depth_loss(
    ...
    def inference_pipeline(
        ...
        start_timestep: int = 0, stop_timestep=None,
        take_time: int = 0, frozen_ref_count: int = 0,
    ):
        ...
        steps_per_inference = self.inference_config["inference_steps"] // (
        ...
        if image_latents is not None:
            ...
            latents = torch.randn(
                latent_shape, generator=self.generator
        ...
        stop_timestep = (
            ...
            if stop_timestep is None else stop_timestep
        )
        ...
        for i in range(start_timestep, stop_timestep):
            ...
            if hasattr(self.test_scheduler, "step_by_indices"):
                ...
                staging_latents = self.test_scheduler.step(
                    noise_pred, timesteps.cpu(), latents
                ).prev_sample
            ...
            if preview_depth and len(model_output) > 1:
                ...
                depth_result.append(
                    ...
                    .flatten(-2).flatten(-4, -2)
                )

        cur_latents = latents[:, take_time].flatten(0, 1).to(dtype=self.vae.dtype)
        if self.is_temporal_vae:
        ...
        result = {
            "images": self.image_processor.postprocess(image_tensor, output_type=output_type),
            "latents": latents
        }
    ...
    def autoregressive_inference_pipeline(self, latent_shape, batch, output_type):
        ...
        if self.inference_config.get("generate_frames_for_reference", True):
            ...
            raw_image_tensor = batch["vae_images"][:, :reference_frame_count]
            image_tensor = self.image_processor.preprocess(
            ...
            if self.is_temporal_vae:
                image_latents = einops.rearrange(
                    image_latents, "(b v) c t h w -> b t v c h w",
                    v=raw_image_tensor.shape[2]
                )
            else:
        ...
        if image_latents is None:
            iteration_batch = {
                k: (
                    v if k in exception_for_take_sequence
                    else dwm.functional.take_sequence_clip(v, 0, iteration_sequence_length)
                )
                for k, v in batch.items()
            }
            stop_timestep = self.inference_config["inference_steps"] - steps_per_inference
        ...
        for i in range(
            ...
            iteration_output = self.inference_pipeline(
                latent_shape, iteration_batch, output_type, image_latents,
                start_timestep=self.inference_config["inference_steps"] - steps_per_inference,
                stop_timestep=self.inference_config["inference_steps"],
                frozen_ref_count=dataset_ref_left
            )

            if self.is_temporal_vae and i == 0:
                result["images"].append(iteration_output["images"].chunk(4)[-1])
            else:
        ...
        if flush_steps == 1:
            for j in range(1, image_latents.shape[1]):
                image_tensor = self.image_processor.postprocess(
                    self.vae.decode(
            ...
            for j in range(1, flush_steps):
                ...
                iteration_output = self.inference_pipeline(
                    latent_shape, iteration_batch, output_type, image_latents,
                    start_timestep=self.inference_config["inference_steps"] + (2 * j - 1) * steps_per_inference,
                    stop_timestep=self.inference_config["inference_steps"] + (2 * j) * steps_per_inference,
                    take_time=j,
                ...
                is_finished = (
                    torch.arange(latent_shape[1], device=self.device) < frozen_i
    ...
    @torch.no_grad()
    def preview_pipeline(self, batch: dict, output_path: str, global_step: int):
        batch_size, sequence_length, view_count = batch["vae_images"].shape[:3]
        ...
        if "sequence_length_per_iteration" in self.inference_config:
            latent_shape = (
                ...
                view_count, self.vae.config.latent_channels,
        ...
        if self.should_save or (
            ...
            if sequence_length == 1:
                image_output_path = os.path.join(output_path, "preview", "{}.png".format(filename))
            ...
            if preview_depth and "depth" in pipeline_output:
                os.makedirs(os.path.join(output_path, "preview_depth"), exist_ok=True)
                video_output_path = os.path.join(output_path, "preview_depth", "{}.mp4".format(filename))
                dwm.utils.preview.save_tensor_to_video(
    ...
    def evaluate_pipeline(
        ...
        for i, batch in enumerate(validation_dataloader):
            ...
            if "fid" in self.metrics:
                fake_images = pipeline_output["images"].unflatten(0, (batch_size, -1, view_count))
                start = 0 if (
                    "sequence_length_per_iteration" not in self.inference_config or
                    self.inference_config.get("generate_frames_for_reference", True)
                ) else self.inference_config.get("reference_frame_count", 1)
                stop = start + fake_images.shape[1]
                self.metrics["fid"].update(
                    batch["vae_images"][:, start:stop].flatten(0, 2).to(self.device),
                    real=True
                )
                self.metrics["fid"].update(
            ...
            if "rmse" in self.metrics:
                preds_target_generator = RollingForcing.enum_depth_preds_and_targets(
        ...
        for k, metric in self.metrics.items():
            ...
            if self.should_save:
                self.summary.add_scalar("evaluation/{}".format(k), value, global_step)
