...
class CrossviewTemporalSD():
    ...
    def encode_vae_images_to_latents(self, vae_images: torch.Tensor):
        ...
        if self.is_temporal_vae:
            ...
            latents = latents.unflatten(0, (batch_size, sequence_length, view_count))

        return latents

    def _make_noised_latents_from_timesteps(
        self,
        clean_latents: torch.Tensor,
        timesteps: torch.Tensor,
        noise: torch.Tensor,
    ):
        if hasattr(self.test_scheduler, "add_noise"):
            return self.test_scheduler.add_noise(clean_latents, noise, timesteps)

        if hasattr(self.test_scheduler, "sigmas") and hasattr(self.test_scheduler, "timesteps"):
            flat_t = timesteps.reshape(-1).to(device=clean_latents.device)
            sched_t = self.test_scheduler.timesteps.to(
                device=clean_latents.device,
                dtype=flat_t.dtype
            )
            diff = (flat_t[:, None] - sched_t[None, :]).abs()
            timestep_indices = diff.argmin(dim=1)

            sigmas = self.test_scheduler.sigmas[timestep_indices].to(
                device=clean_latents.device,
            ...
            sigmas = sigmas.reshape(
                *timesteps.shape,
                *([1] * (clean_latents.ndim - timesteps.ndim))
            )

            return sigmas * noise + (1.0 - sigmas) * clean_latents

        raise NotImplementedError(
            "Current scheduler does not support timestep-based history noising here."
        )

    def _convert_pt_images_output(self, images_pt: torch.Tensor, output_type: str):
        if output_type == "pt":
            return images_pt

        if output_type == "np":
            return images_pt.permute(0, 2, 3, 1).cpu().numpy()

        if output_type == "pil":
            return [
                torchvision.transforms.functional.to_pil_image(img.cpu())
                for img in images_pt
            ]

        raise NotImplementedError(
            f"Unsupported output_type={output_type} in DFoT autoregressive pipe."
        )
    ...
    def inference_pipeline(
        ...
        history_low_noise_step = int(self.inference_config.get("history_low_noise_step", 5))
        history_high_noise_step = int(self.inference_config.get("history_high_noise_step",5))
        history_clean_prob = self.inference_config.get("history_clean_prob", 0.3)
        do_history_guidance = (
        ...
        result = {
            ...
            else self._convert_pt_images_output(images_pt, output_type),
            "latents": latents
        }
    ...
    def autoregressive_inference_pipeline(
        ...
        result = {
            ...
            else self._convert_pt_images_output(merged_images_pt, output_type)
        }
    ...
    def preview_pipeline(
        ...
        if self.should_save or (
            ...
            if preview_frame_count == 1:
                ...
                tok_list = tok_list[:preview_frame_count]
