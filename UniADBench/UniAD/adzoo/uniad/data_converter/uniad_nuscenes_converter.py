...
def create_nuscenes_infos(root_path,
    ...
    Args:
        ...
        max_sweeps (int): Max number of sweeps.
            Default: 10
        val_only (bool): Only generate validation set.
            Default: False
    """
    from nuscenes.nuscenes import NuScenes
    ...
    elif version == 'interp_12Hz_trainval':
        # 12Hz interpolated version, use same scene split as v1.0-trainval
        train_scenes = splits.train
        val_scenes = splits.val
    else:
    ...
    # In val_only mode, clear train_scenes and skip training set processing
    if val_only:
        train_scenes = set()
        print('[val_only mode] Skipping training set generation')
    ...
    if test:
        ...
        # In val_only mode, do not save train pkl
        if not val_only:
            data = dict(infos=train_nusc_infos, metadata=metadata)
            info_path = osp.join(out_path,
...
def _fill_trainval_infos(nusc,
    ...
    # Merge scenes to process for early filtering
    target_scenes = train_scenes | val_scenes
    total_samples = len(nusc.sample)
    processed_samples = 0

    for sample in track_iter_progress(nusc.sample):
        # Early filtering: skip if sample does not belong to target scene
        ...
        # Other samples (e.g. test scenes) will be skipped in val_only mode
