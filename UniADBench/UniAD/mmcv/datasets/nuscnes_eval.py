...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, sample_tokens_filter: list = None):
    """
    ...
    :param sample_tokens_filter: Optional list of sample tokens to filter. If provided, only load GT for these tokens.
    :return: The GT boxes.
    """
    ...
    # If sample_tokens_filter is provided, use it directly instead of splitting by scene
    if sample_tokens_filter is not None:
        if verbose:
            print('Filtering to {} sample tokens from predictions'.format(len(sample_tokens_filter)))
        sample_tokens = [t for t in sample_tokens_filter if t in sample_tokens_all]
        if len(sample_tokens) != len(sample_tokens_filter):
            missing = set(sample_tokens_filter) - set(sample_tokens_all)
            if verbose:
                print('Warning: {} sample tokens not found in database'.format(len(missing)))
    else:
    ...
    # Build index_map for all scenes (needed for tracking)
    index_map = {}
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                     verbose=verbose)
        # Use prediction sample_tokens to filter GT (supports subset evaluation)
        self.gt_boxes = load_gt(self.nusc, self.eval_set, DetectionBox_modified, verbose=verbose,
                                sample_tokens_filter=self.pred_boxes.sample_tokens)

        # Note: We don't need strict equality check when using filter, as some pred tokens may not have GT
        # assert set(self.pred_boxes.sample_tokens) == set(self.gt_boxes.sample_tokens), \
        #     "Samples in split doesn't match samples in predictions."
        if verbose:
            print(f'Loaded {len(self.pred_boxes.sample_tokens)} predictions, {len(self.gt_boxes.sample_tokens)} GT samples')
