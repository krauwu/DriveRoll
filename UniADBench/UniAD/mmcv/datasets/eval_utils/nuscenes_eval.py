...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, data_infos=None):
    """
    ...
    :param data_infos: Optional list of data infos to filter GT samples.
    :return: The GT boxes.
    """
    ...
    # If data_infos is provided, only load sample_tokens from it
    data_tokens = None
    if data_infos is not None and len(data_infos) > 0:
        data_tokens = set()
        ...
        if verbose:
            print('Filtering GT by data_infos: {} samples'.format(len(data_tokens)))
    ...
    for sample_token in sample_tokens_all:
        ...
        if scene_record['name'] in splits[eval_split]:
            # If data_tokens is specified, only keep samples in it
            if data_tokens is None or sample_token in data_tokens:
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                     verbose=verbose)
        self.gt_boxes = load_gt(self.nusc, self.eval_set, DetectionBox_modified, verbose=verbose, data_infos=self.data_infos)
