    ...
    def evaluate(self,
        ...
        if isinstance(results, dict):
            ...
            if 'planning_results_computed' in results.keys():
                ...
                # 18 frames correspond to 3 seconds (6Hz data), key frame indices: 3,6,9,12,15,18 (corresponding to 0.5s to 3.0s)
                key_indices = [2, 5, 8, 11, 14, 17]  # 0-indexed
                for key in planning_results_computed.keys():
                    value = planning_results_computed[key]
                    row_value = []
                    row_value.append(key)
                    # If value count > 6, only take key time points
                    if len(value) > 6:
                        for idx in key_indices:
                            if idx < len(value):
                                row_value.append('%.4f' % float(value[idx]))
                            else:
