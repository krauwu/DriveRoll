...
class NuScenesTraj(object):
    def __init__(self,
        ...
        # GT sampling frequency: 2Hz, 6Hz, 12Hz
        self.gt_fps = gt_fps
    ...
    def get_sdc_traj_label(self, sample_token):
        ...
        if self.gt_fps == 2:
            ...
            # 2Hz no sampling needed

        elif self.gt_fps == 6:
            for _ in range(self.predict_steps*3):
                next_annotation_token = sd_rec['next']
                if next_annotation_token == '':
                    break
                sd_rec = self.nusc.get('sample', next_annotation_token)
                ...
                ego_pose_next = self.nusc.get('ego_pose', lidar_top_data_next['ego_pose_token'])
                sdc_fut_traj.append(ego_pose_next['translation'][:2])
            # 6Hz downsampling: [0, 3, 6, 9, 12, 15]
            sample_indices =  [2,5,8,11,14,17,20,23,26,29,32,35]
            sdc_fut_traj = [sdc_fut_traj[i] for i in sample_indices if i < len(sdc_fut_traj)]

        elif self.gt_fps == 12:
            for _ in range(self.predict_steps*6):
                next_annotation_token = sd_rec['next']
                if next_annotation_token == '':
                    break
                sd_rec = self.nusc.get('sample', next_annotation_token)
                ...
                ego_pose_next = self.nusc.get('ego_pose', lidar_top_data_next['ego_pose_token'])
                sdc_fut_traj.append(ego_pose_next['translation'][:2])
            # 12Hz downsampling: [0, 6, 12, 18, 24, 30]
            sample_indices = [5,11,17,23,29,35,41,47,53,59,65,71]
            sdc_fut_traj = [sdc_fut_traj[i] for i in sample_indices if i < len(sdc_fut_traj)]
    ...
    def get_sdc_planning_label(self, sample_token):
        ...
        if self.gt_fps == 2:
            ...
            for _ in range(self.planning_steps * 3):
                next_annotation_token = sd_rec['next']
                if next_annotation_token == '':
                    break
                sd_rec = self.nusc.get('sample', next_annotation_token)
                l2e_r_mat_curr, l2e_t_curr, e2g_r_mat_curr, e2g_t_curr = self.get_l2g_transform(
                    sd_rec)  # (lidar to global at current frame)

                # bbox of sdc under current lidar frame
                next_bbox3d = self.generate_sdc_info(self.sdc_vel_info[next_annotation_token],
                ...
                planning.append(next_bbox3d)
            # 6Hz downsampling: [0, 3, 6, 9, 12, 15]
            sample_indices = [2,5,8,11,14,17]
            planning = [planning[i] for i in sample_indices if i < len(planning)]

        elif self.gt_fps == 12:
            for _ in range(self.planning_steps * 6):
                next_annotation_token = sd_rec['next']
                if next_annotation_token == '':
                    break
                sd_rec = self.nusc.get('sample', next_annotation_token)
                ...
                planning.append(next_bbox3d)
            # 12Hz downsampling: [0, 6, 12, 18, 24, 30]
            sample_indices = [5,11,17,23,29,35]
            planning = [planning[i] for i in sample_indices if i < len(planning)]
