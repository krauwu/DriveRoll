...
class UniAD(UniADTrack):
    ...
    def forward_test(self,
        ...
        if self.with_seg_head:
            # Decide whether to keep pts_bbox based on ENABLE_PLOT_MODE (for visualization)
        ...
        if self.with_occ_head:
            # Decide whether to keep pred_ins_sigmoid based on ENABLE_PLOT_MODE (for per-vehicle occupancy visualization)
            if os.environ.get('ENABLE_PLOT_MODE', None) is None:
