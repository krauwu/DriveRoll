...
# ========= 路径硬编码（与 cfg 一致）=========
NUSC_ROOT = "<path-to-local-resource>"
train_data_pkl = "<path-to-local-resource>"
val_data_pkl = "<path-to-local-resource>"
IMG_DESC_PATH = "<path-to-local-resource>"
...
def make_base_ds(train=False):
    ...
    return NuPlanDataset(
        ...
        pkl_path=data_pkl,
        cache_root='<path-to-local-resource>',
        dataset_root='<path-to-local-resource>',
        map_root='<path-to-local-resource>',
        sequence_length=30,
