...
def _filter_blob_with_sensor_ori(sensor_blobs_root, db_root, logs):
    """
...
def _filter_blob_with_sensor(sensor_blobs_root, db_root, logs):
    """
    sensor_blobs_root: 例如 .../nuPlan/sensor_blobs
      - 支持两种结构:
        1. 直接包含 log 文件夹
        2. 下面有 shard 目录（如 nuplan-v1.1_mini_camera_0..7），每个里面有 log 文件夹
    db_root: 放 .db 的目录
    logs: 你 splits 里的 log 名列表
    """
    sensor_exist = set()
    
    for item in os.listdir(sensor_blobs_root):
        item_path = osp.join(sensor_blobs_root, item)
        if not osp.isdir(item_path):
            continue
        
        # 检查是否是 log 文件夹（log 名通常以日期格式开头，如 2021.xx.xx）
        if item.startswith("202"):
            # 直接是 log 文件夹
            sensor_exist.add(item)
        else:
            # 是 shard 目录，遍历其下的 log 文件夹
            for d in os.listdir(item_path):
                if osp.isdir(osp.join(item_path, d)):
                    sensor_exist.add(d)

    db_exist = set(os.listdir(db_root))
    
    result = [name for name in logs if name in sensor_exist and (name + ".db") in db_exist]
    
    # 添加调试信息
    print(f"sensor_exist count: {len(sensor_exist)}")
    print(f"db_exist count: {len(db_exist)}")
    print(f"matched logs: {len(result)}")
...
if __name__ == "__main__":
    ...
    dataset_root = "../data/nuplan"
    data_root = osp.join(dataset_root, "nuplan-v1.1", "splits", "mini")   #  .db 目录
    map_root = osp.join(dataset_root, "maps")
    out_path = "./outputs/nuplan-test"
