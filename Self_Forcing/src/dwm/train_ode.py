"""
ODE 训练脚本

基于 train.py 修改，使用 ODE 轨迹数据进行训练
主要区别：
- 使用 ODENuScenesDataset 加载预生成的 ODE 轨迹数据
- 训练时使用 pipeline.train_step_ode 替代 pipeline.train_step

训练命令：
CUDA_VISIBLE_DEVICES=3 PYTHONPATH=src:externals/waymo-open-dataset/src:externals/TATS/tats/fvd torchrun --master_port=29501 --nproc_per_node=1 -m dwm.train_ode -c configs/train_ode/default.json -o output/ode_train --data-folder output/nuscenes_ode_pairs

"""

import argparse
import dwm.common
import json
import os
from tqdm import tqdm
import debugpy
import torch
from dwm.utils.sampler import VariableVideoBatchSampler
from dwm.datasets.common import CollateFnIgnoring

def create_parser():
    parser = argparse.ArgumentParser(
        description="The script to finetune a stable diffusion model using ODE loss.")
    parser.add_argument(
        "-c", "--config-path", type=str, required=True,
        help="The config to load the train model and dataset.")
    parser.add_argument(
        "-o", "--output-path", type=str, default=None,
        help="The path to save checkpoint files.")
    parser.add_argument(
        "--log-steps", default=100, type=int,
        help="The step count to print log and update the tensorboard.")
    parser.add_argument(
        "--preview-steps", default=200, type=int,
        help="The step count to preview the pipeline result.")
    parser.add_argument(
        "--checkpointing-steps", default=1000, type=int,
        help="The step count to save the checkpoint.")
    parser.add_argument(
        "--evaluation-steps", default=10000, type=int,
        help="The step count to preview the pipeline result.")
    parser.add_argument(
        "--resume-from", default=None, type=int,
        help="The step to resume from")
    parser.add_argument(
        "--wandb", action="store_true",
        help="Use wandb to log the training process.")
    parser.add_argument(
        "--wandb-project", type=str, default="dwm_ode",
        help="The wandb project name.")
    parser.add_argument(
        "--wandb-run-name", type=str, default="train_ode",
        help="The wandb run name.")
    parser.add_argument(
        "--data-folder", type=str, required=True,
        help="The folder containing ODE trajectory .pt files.")
    parser.add_argument(
        "--denoising-step-list", type=str, default="1000,748,502,247,0",
        help="Comma-separated list of denoising steps for training.")
    return parser


if __name__ == "__main__":
    parser = create_parser()
    args = parser.parse_args()

    # debugpy.listen(("0.0.0.0", 9876))
    # print("[debugpy] listening on, waiting for VS Code to attach...")
    # debugpy.wait_for_client()
    # print("attached")

    with open(args.config_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    # 解析 denoising_step_list
    denoising_step_list = [int(x) for x in args.denoising_step_list.split(",")]

    torch.manual_seed(config["generator_seed"])

    # set distributed training (if enabled), log, random number generator, and
    # load the checkpoint (if required).
    ddp = "LOCAL_RANK" in os.environ
    if ddp:
        local_rank = int(os.environ["LOCAL_RANK"])
        device = torch.device(config["device"], local_rank)
        if config["device"] == "cuda":
            torch.cuda.set_device(local_rank)

        torch.distributed.init_process_group(backend=config["ddp_backend"])
    else:
        device = torch.device(config["device"])

    # setup the global state
    if "global_state" in config:
        for key, value in config["global_state"].items():
            dwm.common.global_state[key] = \
                dwm.common.create_instance_from_config(value)

    should_log = (ddp and local_rank == 0) or not ddp
    should_save = not torch.distributed.is_initialized() or \
        torch.distributed.get_rank() == 0

    # load the pipeline including the models
    output_path = config["output_path"] if args.output_path is None else args.output_path
    pipeline = dwm.common.create_instance_from_config(
        config["pipeline"], output_path=output_path, config=config,
        device=device, resume_from=args.resume_from)

    # 设置 denoising_step_list 到 pipeline（如果支持）
    if hasattr(pipeline, "set_denoising_step_list"):
        pipeline.set_denoising_step_list(denoising_step_list)

    if should_log:
        print("The pipeline is loaded.")
        print(f"Training with denoising steps: {denoising_step_list}")

    if args.wandb and should_save:
        import wandb
        wandb.init(project=args.wandb_project, name=args.wandb_run_name, config=config)

    # load the dataset - 使用 ODE 数据集
    from dwm.datasets.ode_nuscenes import ODENuScenesDataset

    # Dataloader 配置（直接定义）
    batch_size = config.get("batch_size", 1)
    num_workers = config.get("num_workers", 4)
    pin_memory = config.get("pin_memory", True)

    # 创建 ODE 训练数据集
    training_dataset = ODENuScenesDataset(
        data_folder=args.data_folder,
        split="train",
        denoising_step_list=denoising_step_list
    )


    # 创建 ODE 验证数据集
    validation_dataset = ODENuScenesDataset(
        data_folder=args.data_folder,
        split="val",
        denoising_step_list=denoising_step_list
    )

    if ddp:
        # if "mix_config" in config.keys():
        if 0:
            process_group = torch.distributed.group.WORLD

            training_datasampler = VariableVideoBatchSampler(
                training_dataset,
                config["mix_config"],
                num_replicas=process_group.size(),
                rank=process_group.rank(),
                shuffle=config["data_shuffle"],
                seed=config["generator_seed"],
                collate_fn=CollateFnIgnoring(['clip_text'])
            )

            training_dataloader = torch.utils.data.DataLoader(
                training_dataset,
                batch_sampler=training_datasampler,
                num_workers=num_workers,
                pin_memory=pin_memory,
                collate_fn=CollateFnIgnoring(['clip_text'])
            )

        else:
            training_datasampler = torch.utils.data.distributed.DistributedSampler(
                training_dataset, shuffle=config["data_shuffle"],
                seed=config["generator_seed"])
            training_dataloader = torch.utils.data.DataLoader(
                training_dataset,
                batch_size=batch_size,
                sampler=training_datasampler,
                num_workers=num_workers,
                pin_memory=pin_memory,
                collate_fn=CollateFnIgnoring(['clip_text'])
            )

        # make equal sample count for each process to simplify the result gathering
        total_batch_size = int(os.environ["WORLD_SIZE"]) * batch_size
        dataset_length = len(validation_dataset) // total_batch_size * total_batch_size
        validation_dataset = torch.utils.data.Subset(
            validation_dataset, range(0, dataset_length))
        validation_datasampler = torch.utils.data.distributed.DistributedSampler(
            validation_dataset, shuffle=False)
        validation_dataloader = torch.utils.data.DataLoader(
            validation_dataset,
            batch_size=batch_size,
            sampler=validation_datasampler,
            num_workers=num_workers,
            pin_memory=pin_memory,
            collate_fn=CollateFnIgnoring(['clip_text']))
    else:
        training_dataloader = torch.utils.data.DataLoader(
            training_dataset,
            batch_size=batch_size,
            shuffle=config["data_shuffle"],
            num_workers=num_workers,
            pin_memory=pin_memory,
            collate_fn=CollateFnIgnoring(['clip_text'])
        )
        validation_datasampler = None
        validation_dataloader = torch.utils.data.DataLoader(
            validation_dataset,
            batch_size=batch_size,
            shuffle=False,
            num_workers=num_workers,
            pin_memory=pin_memory,
            collate_fn=CollateFnIgnoring(['clip_text'])
        )

    preview_dataloader = torch.utils.data.DataLoader(
        validation_dataset,
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=pin_memory,
        collate_fn=CollateFnIgnoring(['clip_text'])
    )
    preview_data_iterator = iter(preview_dataloader)

    if should_log:
        print("The training dataset is loaded with {} items.".format(
            len(training_dataset)))
        print("The validation dataset is loaded with {} items.".format(
            len(validation_dataset)))

    # train loop
    global_step = 0 if args.resume_from is None else args.resume_from
    for epoch in range(config["train_epochs"]):

        if ddp:
            # Fixing training data order reduces the accessed objects per rank,
            # therefore reduces the upper-bound of memory usage comsumed by the
            # Python reference counting of objects.
            sampler_epoch = 0 if config.get("fix_training_data_order", False) \
                else epoch
            training_datasampler.set_epoch(sampler_epoch)

        loader = tqdm(
            training_dataloader,
            total=len(training_dataloader),
            desc=f"Epoch {epoch}",
            dynamic_ncols=True
        )

        for batch in loader:
            # 使用 ODE 训练步骤
            pipeline.train_step_ode(batch, global_step)
            global_step += 1

            # for debug
            if global_step % 10 == 0 or global_step == 1:
                pipeline.preview_ode_pipeline(batch, output_path, global_step)
                pipeline.log(global_step, args.log_steps)
                try:
                    preview_batch = next(preview_data_iterator)
                except StopIteration:
                    preview_data_iterator = iter(preview_dataloader)
                    preview_batch = next(preview_data_iterator)

                pipeline.preview_ode_pipeline(preview_batch, output_path, str(global_step)+"_val")

            # log
            # if global_step % args.log_steps == 0:
                # pipeline.log(global_step, args.log_steps)

            # # preview
            # if global_step % args.preview_steps == 0:
            #     if preview_dataloader is None:
            #         pipeline.preview_ode_pipeline(batch, output_path, global_step)
            #     else:
            #         try:
            #             preview_batch = next(preview_data_iterator)
            #         except StopIteration:
            #             preview_data_iterator = iter(preview_dataloader)
            #             preview_batch = next(preview_data_iterator)

            #         pipeline.preview_ode_pipeline(preview_batch, output_path, global_step)

            # save step checkpoint
            if global_step % args.checkpointing_steps == 0:
                pipeline.save_checkpoint(output_path, global_step)

            # # evaluation
            # if (
            #     args.evaluation_steps > 0 and
            #     global_step % args.evaluation_steps == 0
            # ):
            #     pipeline.evaluate_pipeline(
            #         global_step, len(validation_dataset),
            #         validation_dataloader, validation_datasampler)

        if should_log:
            print("Epoch {} done.".format(epoch))

    if torch.distributed.is_initialized():
        torch.distributed.destroy_process_group()