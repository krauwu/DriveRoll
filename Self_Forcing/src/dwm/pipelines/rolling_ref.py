...
class CrossviewTemporalSD():
    ...
    def set_denoising_step_list(self, denoising_step_list):
        """设置 ODE 训练使用的去噪步数列表"""
    ...
    def train_step_ode(self, batch: dict, global_step: int):
        """ODE 训练步骤
        使用预计算的 ODE 轨迹进行回归训练
        batch 应包含 "ode_latent": [batch, num_selected, seq, view, c, h, w]
        """
        self.model_wrapper.train()
        ...
        # ode_latent: [batch, num_selected, seq, view, c, h, w]
        batch_size, num_selected, sequence_length, view_count, num_channels, _, _ = ode_latent.shape

        # target 是最干净的 latent (最后一个是 clean)
        target_latent = ode_latent[:, -1]  # [batch, seq, view, c, h, w]

        # 随机选择 timestep 对于每个 batch 和每个 frame
        # 不选择最后一个（clean）
        ...
        noisy_input = torch.gather(
            ode_latent,
            dim=1,
            index=timestep_index.reshape(batch_size, 1, sequence_length, view_count, 1, 1, 1)
                 .expand(-1, -1, -1, -1, num_channels, ode_latent.shape[-2], ode_latent.shape[-1])
        ).squeeze(1)  # [batch, seq, view, c, h, w]

        # 获取对应的 timestep 值
        timestep_values = torch.randint(
            0, self.train_scheduler.config.num_train_timesteps,
            (batch_size, sequence_length, view_count),
            generator=self.generator,
            device=self.device
        ...
        if isinstance(self.model, diffusers.SD3Transformer2DModel):
            ...
            # 添加噪声
            noise = torch.randn_like(target_latent, generator=self.generator).to(self.device)
            sigmas = CrossviewTemporalSD.sd3_get_sigmas(
                self.train_scheduler, timestep_indices, n_dim=target_latent.ndim,
                dtype=target_latent.dtype, device=target_latent.device)
            noisy_input = sigmas * noise + (1.0 - sigmas) * target_latent
            target = target_latent
        else:
            # UNet 模型处理
            timesteps = timestep_values.clone()
            noisy_input = noisy_input.clone()
            if self.train_scheduler.config.prediction_type == "epsilon":
                target = torch.randn_like(target_latent, generator=self.generator).to(self.device)
                noise = target
                # 计算噪声添加
                noise_scale = (timesteps / self.train_scheduler.config.num_train_timesteps).float()
                noise_scale = noise_scale.reshape(batch_size, sequence_length, view_count, 1, 1, 1)
                # 简单的噪声添加近似
                alpha = 1.0 - noise_scale
                noisy_input = alpha * target_latent + noise_scale * noise
            elif self.train_scheduler.config.prediction_type == "v_prediction":
                # v-prediction 处理
                target_latent_for_v = target_latent
                noise = torch.randn_like(target_latent, generator=self.generator).to(self.device)
                alpha = 1.0 - (timesteps / self.train_scheduler.config.num_train_timesteps).float().reshape(batch_size, sequence_length, view_count, 1, 1, 1)
                noisy_input = alpha * target_latent_for_v + (1.0 - alpha) * noise
                target = noise
            else:
                raise ValueError(f"Unsupported prediction type: {self.train_scheduler.config.prediction_type}")

        # 确保 timesteps 形状正确: [batch, seq, view]
        while len(timesteps.shape) < 3:
            timesteps = timesteps.unsqueeze(-1).repeat_interleave(
                [batch_size, sequence_length, view_count][len(timesteps.shape)], -1)
        ...
        with self.get_autocast_context():
            # 准备条件 - 需要从 batch 中提取条件信息
            # 为了兼容，构建一个虚拟的 vae_images 用于 get_conditions
            dummy_vae_images = torch.zeros(
                batch_size, sequence_length, view_count, 3,
                noisy_input.shape[-2] * 2 ** (len(self.vae.config.down_block_types) - 1),
                noisy_input.shape[-1] * 2 ** (len(self.vae.config.down_block_types) - 1),
            ...
            model_conditions = CrossviewTemporalSD.get_conditions(
                ...
                self.common_config, dummy_vae_images.shape, batch,
                self.device, self.model_dtype, text_condition_mask,
                _3dbox_condition_mask, hdmap_condition_mask,
                action_condition_mask, explicit_view_modeling_mask,
                latents_shape=target_latent.shape)

            # 参考 frame 处理
            noisy_latents, timesteps, additional_conditions, \
                reference_frame_indicator = \
                CrossviewTemporalSD.try_make_input_for_prediction(
                    noisy_input, target_latent, timesteps,
                    self.training_config, self.common_config,
                    generator=self.generator,
                    reference_latent_count=reference_latent_count)
            if additional_conditions is not None:
            ...
            # 处理预测结果
            if isinstance(self.model, diffusers.SD3Transformer2DModel):
                # SD3 的 flow matching 公式
                sd_pred_latent = sd_pred[0] * (-sigmas) + noisy_latents
            else:
                sd_pred_latent = sd_pred[0]
            ...
            loss_dict["ode_loss"] = torch.nn.functional.mse_loss(
                sd_pred_latent.float(), target.float(), reduction="mean"
            ) * self.get_loss_coef("ode")
