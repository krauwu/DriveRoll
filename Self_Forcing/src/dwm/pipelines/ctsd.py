...
class CrossviewTemporalSD():
    ...
    def __init__(
        ...
        if (
            ...
            distribution_framework == "ddp" or
            distribution_framework == "none"
        ):
        ...
        if self.training_config.get("enable_grad_scaler", False):
            if (
                not torch.distributed.is_initialized() or
                self.distribution_framework == "ddp" or
                self.distribution_framework == "none"
            ):
        ...
        if torch.distributed.is_initialized():
            if self.distribution_framework == "ddp":
                ...
                self.model_wrapper = FSDP(
                    ...
                    **self.common_config["ddp_wrapper_settings"])
            elif self.distribution_framework == "none":
                # 不做分布式包装，模型直接加载到本地 GPU（用于 DMD 中 real_score/fake_score）
                pass
            else:
                raise Exception(
        ...
        if self.should_save:
            if len(self.loss_report_list) > 0 and \
                ...
                loss_value = sum(self.loss_report_list) / \
                    (len(self.loss_report_list)+1e-6)
                print(
    ...
    def train_step_ode(self, batch: dict, global_step: int):
        self.model_wrapper.train()

        t0 = time.time()

        ode_latent = batch["ode_latent"].to(self.device)
        batch_size, noise_stage_num, sequence_length, view_count = ode_latent.shape[:4]
        timestep_stage = batch["timestamps"].to(self.device)
        # timestep_stage_indice = batch['ODE_TIME']
        device = self.device
        # reference_latent_count = self.get_reference_latent_count()

        def random_stage_noise():
            # 1. 随机选择 k（condition frame 数量）
            reference_latent_count = torch.randint(0, 3, (1,)).item()
            k = reference_latent_count

            # 2. 初始化选择的索引 [batch, sequence_length]
            noise_stage_indices = torch.zeros(batch_size, sequence_length,
                                                dtype=torch.long, device=device)

            # 3. 前 k 帧用 clean
            noise_stage_indices[:, :k] = noise_stage_num - 1

            # 4. 剩下的帧随机选择 noise stage（0 到 noise_stage_num-2）
            noise_stage_indices[:, k:] = torch.randint(
                0, noise_stage_num - 1,
                (batch_size, sequence_length - k),
                device=device
            )

            # 5. 扩展到 view 维度 [batch, sequence_length, 1]，用于 gather
            expand_idx = noise_stage_indices.unsqueeze(1)  # [batch, 1, seq]

            return expand_idx, reference_latent_count

        def fix_stage_noise(cond_len=3):
            k = cond_len
        ...
        expand_idx, reference_latent_count = fix_stage_noise(cond_len=self.inference_config.get("reference_frame_count", 3))  ## for single frame roll out, 默认是3
        

        # 6. 从 ode_latent 中取出 [batch, seq, view, c, h, w]
        num_channels, h, w = ode_latent.shape[4:]
        noisy_latents = torch.gather(
            ode_latent,
            dim=1,
            index=expand_idx.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                .expand(-1, -1, -1, view_count, num_channels, h, w)
        ).squeeze(1)  # [batch, seq, view, c, h, w]

        # 7. 获取对应的 timestamps [batch, seq]
        timesteps = timestep_stage.unsqueeze(-1).expand(-1, -1, sequence_length).gather(1, expand_idx).squeeze(1)
        latents = ode_latent[:, -1] # b t v c h w


        target = latents
        distances = (self.train_scheduler.timesteps.unsqueeze(1) - timesteps.unsqueeze(0).cpu()).abs()
        timestep_indices = distances.argmin(dim=1)
        ...
        # prepare conditions, in ode distillation, cfg is not necessary
        text_condition_mask = (
            torch.rand((batch_size,), generator=self.generator) <
            1.0)\
            .tolist()
        ...
        with self.get_autocast_context():
            model_conditions = CrossviewTemporalSD.get_conditions(
                ...
                self.common_config, latents.shape, batch,
                self.device, self.model_dtype, text_condition_mask,
                _3dbox_condition_mask, hdmap_condition_mask,
                action_condition_mask, explicit_view_modeling_mask,
                latents_shape=latents.shape)
    ...
    def train_step_critic(self, batch: dict, latents: torch.Tensor, global_step: int, reference_frame_count: int = 0):
        self.model_wrapper.train()
        ...
        if isinstance(self.model, diffusers.UNetSpatioTemporalConditionModel):
            ...
            u = CrossviewTemporalSD.sd3_compute_density_for_timestep_sampling(
                ...
                size=latents.shape[:timestep_shape_range],
                logit_mean=0.0, logit_std=1.0, mode_scale=1.29)
            ## cyr, used in self forcing, all frames are noised in the same stage
            u = torch.ones_like(u)*u[0,0]
            timestep_indices = (
                u * self.train_scheduler.config.num_train_timesteps
            ).long()
            timesteps = self.train_scheduler.timesteps[timestep_indices]\
            ...
            # skip_reference_noise: 对前 reference_frame_count 帧不加噪，保留 clean latent
            if reference_frame_count > 0:
                noisy_latents[:, :reference_frame_count] = latents[:, :reference_frame_count]
                timesteps[:, :reference_frame_count] = 0
                sigmas[:, :reference_frame_count] = 0
        ...
        with self.get_autocast_context():
            ...
            # skip_reference_noise: 只对非 reference 帧计算 critic loss
            if reference_frame_count > 0:
                reference_frame_loss_mask = torch.ones(
                    *sd_pred_latent.shape[:3], 1, 1, 1, dtype=torch.bool, device=sd_pred_latent.device)
                reference_frame_loss_mask[:, :reference_frame_count] = False
                sd_pred_latent = sd_pred_latent * reference_frame_loss_mask.float()
                target = target * reference_frame_loss_mask.float()
        ...
        self.step_duration += time.time() - t0
        return sd_pred_latent.detach(), noisy_latents, target

    @torch.no_grad()
    def compute_x0(self, batch: dict, latents: torch.Tensor, timestep: int, guidance_scale=0, reference_frame_count: int = 0):
        '''
        在train dmd -> compute kl 中使用:给latent加一个随机强度的噪声然后单步去噪得到x0。

        Args:
            reference_frame_count: reference 帧数量，>0 时不对前 k 帧加噪，直接保留 clean latent
        '''
        self.model_wrapper.eval()

        batch_size, sequence_length, view_count = latents.shape[:3]
        do_classifier_free_guidance = guidance_scale > 0  # 小于等于0时默认为1

        # latents = latents.unflatten(0, batch["vae_images"].shape[:3])
        noise = torch.randn(
            latents.shape, generator=self.generator).to(self.device)

        timesteps = timestep.reshape(*latents.shape[:2], 1).repeat(1, 1, view_count)
        sigmas = timesteps[..., None, None, None]/1000 ## cyr, hard code
        noisy_latents = sigmas * noise + (1.0 - sigmas) * latents

        # skip_reference_noise: 对前 reference_frame_count 帧不加噪，保留 clean latent，timestep 和 sigma 也设为 0
        if reference_frame_count > 0:
            noisy_latents[:, :reference_frame_count] = latents[:, :reference_frame_count]
            timesteps[:, :reference_frame_count] = 0
            sigmas[:, :reference_frame_count] = 0
        ...
        with self.get_autocast_context():
            ...
            sd_pred = sd_pred[0]
            if do_classifier_free_guidance:
                sd_pred_uncond, sd_pred_cond = sd_pred.chunk(2)
                sd_pred = sd_pred_uncond + guidance_scale * \
                    (sd_pred_cond - sd_pred_uncond)

            sd_pred_latent = sd_pred * (-sigmas) + noisy_latents \
                if isinstance(self.model, diffusers.SD3Transformer2DModel) \
                else sd_pred
            
            other_result_dict = {"noisy_latents": noisy_latents}
            
            return sd_pred_latent, other_result_dict


    @torch.no_grad()
    def preview_ode_pipeline(self, batch: dict, output_path: str, global_step: int):
        """ODE 可视化函数
        基于 train_step_ode 的逻辑，但不计算 loss，将预测和输入的 latent 都 decode 为 rgb 可视化

        Args:
            batch: 包含 "ode_latent" 的 batch
            output_path: 输出路径
            global_step: 当前 step
        """
        self.model_wrapper.eval()
        ...
        expand_idx, reference_latent_count = fix_stage_noise(cond_len=self.inference_config.get("reference_frame_count", 3))
        
        # 6. 扩展到 [batch, seq, view, c, h, w] 用于 gather
        num_channels, h, w = ode_latent.shape[4:]
        noisy_latents = torch.gather(
            ode_latent,
            dim=1,
        ...
        # 7. 获取对应的 timestamps [batch, seq]
        timesteps = timestep_stage.unsqueeze(-1).expand(-1, -1, sequence_length).gather(1, expand_idx).squeeze(1)
        latents = ode_latent[:, -1]  # clean latent: [batch, seq, view, c, h, w]
        ...
        # 确保 timesteps 形状为 [batch, seq, view]
        while len(timesteps.shape) < 3:
            timesteps = timesteps.unsqueeze(-1).repeat_interleave(latents.shape[len(timesteps.shape)], -1)
        ...
        model_conditions = CrossviewTemporalSD.get_conditions(
            self.model,
            self.text_encoders if isinstance(self.model, diffusers.SD3Transformer2DModel) else self.text_encoder,
            self.tokenizers if isinstance(self.model, diffusers.SD3Transformer2DModel) else self.tokenizer,
            self.common_config, latents.shape, batch,
            device, self.model_dtype, text_condition_mask,
            _3dbox_condition_mask, hdmap_condition_mask,
            action_condition_mask, explicit_view_modeling_mask,
            latents_shape=latents.shape)

        noisy_latents, timesteps, additional_conditions, _ = \
            CrossviewTemporalSD.try_make_input_for_prediction(
        ...
        # 前向传播，获取预测
        sd_pred, _, _ = self.model_wrapper(noisy_latents, timesteps, **model_conditions)

        # SD3 flow matching: v -> x_0
        sd_pred_latent = sd_pred[0] * (-sigmas) + noisy_latents \
            if isinstance(self.model, diffusers.SD3Transformer2DModel) \
            else sd_pred[0]

        # VAE decode 参数
        shift_factor = self.vae.config.shift_factor if self.vae.config.shift_factor is not None else 0
        is_temporal_vae = self.is_temporal_vae

        def decode_latents(latents_to_decode):
            """Decode latents to RGB images"""
            if is_temporal_vae:
                view_count = latents_to_decode.shape[2]
                cur_latents = einops.rearrange(
                    latents_to_decode, "b t v c h w -> (b v) c t h w")
            else:
                cur_latents = latents_to_decode.flatten(0, 2)
            ...
            return self.image_processor.postprocess(image_tensor, output_type="pt")

        # Decode: target (ground truth clean), prediction, input (noisy)
        target_images = decode_latents(target) # [B, T, V, C, H, W]
        target_images = target_images.reshape(batch_size, sequence_length, view_count, *target_images.shape[-3:]) 
        pred_images = decode_latents(sd_pred_latent)
        pred_images = pred_images.reshape(batch_size, sequence_length, view_count, *pred_images.shape[-3:]) 
        input_images = decode_latents(noisy_latents)
        input_images = input_images.reshape(batch_size, sequence_length, view_count, *input_images.shape[-3:]) 

        # --- 修改后的可视化逻辑 ---
        if self.should_save or (torch.distributed.is_initialized() and self.inference_config.get("all_rank_preview", False)):
            os.makedirs(os.path.join(output_path, "preview_ode"), exist_ok=True)
            filename = f"{global_step}_{torch.distributed.get_rank()}" if self.inference_config.get("all_rank_preview", False) else str(global_step)

            # 我们只取 batch 中的第一个样本进行预览 (b=0)
            b = 0 
            frames_for_video = []
            ...
            for t in range(sequence_length):
                ...
                for v in range(view_count):
                    # 获取当前帧、当前视角的三个图像
                    img_input = input_images[b, t, v]   # [C, H, W]
                    img_pred = pred_images[b, t, v]
                    img_target = target_images[b, t, v]
                    
                    # 按顺序加入列表：[In_v0, Pred_v0, Target_v0, In_v1, Pred_v1, Target_v1, ...]
                    frame_views.extend([img_input, img_pred, img_target])
                
                # 将当前时刻的所有视角拼成一张大图 (nrow=3 保证了每行是 Input, Pred, Target)
                # padding=2 增加一点间隔方便观察
                grid_frame = torchvision.utils.make_grid(frame_views, nrow=3, padding=2)
                frames_for_video.append(grid_frame)

            # 转换为 Tensor: [T, C, H_new, W_new]
            video_tensor = torch.stack(frames_for_video)

            if sequence_length == 1:
                # 单帧情况
                image_path = os.path.join(output_path, "preview_ode", f"{filename}.png")
                torchvision.transforms.functional.to_pil_image(video_tensor[0]).save(image_path)
            else:
                # 多帧视频情况
                video_path = os.path.join(output_path, "preview_ode", f"{filename}.mp4")
                # 强制设置 fps=2
                dwm.utils.preview.save_tensor_to_video(
                    video_path, "libx264", 2, video_tensor
                )
    @torch.no_grad()
    def decode_latents(self, latents_to_decode):
        """Decode latents to RGB images"""
        shift_factor = self.vae.config.shift_factor if self.vae.config.shift_factor is not None else 0
        if self.is_temporal_vae:
            view_count = latents_to_decode.shape[2]
    ...
    def inference_pipeline_few_step(
        self, latent_shape, batch, output_type, image_latents=None,
        reference_frame_count: int = 3, selected_timesteps: list = [1000, 0], take_time: int = 0, frozen_ref_count: int = 0, is_train=True
    ):
        '''
        这个函数主要用于dmd
        '''
        ...
        # 若没有参考帧latents则从gt image中计算
        shift_factor = self.vae.config.shift_factor \
            if self.vae.config.shift_factor is not None else 0
        if image_latents is None and reference_frame_count > 0:
            raw_image_tensor = \
        ...
        if image_latents is not None:
            latents = torch\
                .randn(latent_shape, generator=self.generator).to(self.device) * \
                getattr(self.test_scheduler, "init_noise_sigma", 1)
            latents[:,:reference_frame_count] = image_latents[:,:reference_frame_count]
        else:
            # full sequence denoising
            latents = torch\
                .randn(latent_shape, generator=self.generator).to(self.device) * \
        ...
        for i, t in enumerate(selected_timesteps[:-1]):
            ## follow self forcing, 只在最后一步denoise时打开grad
            with torch.set_grad_enabled((i==len(selected_timesteps)-2) and is_train):
                timesteps = torch.ones(*latent_shape[:3], device=latents.device)*t
                timesteps_next = torch.ones(*latent_shape[:3], device=latents.device)*selected_timesteps[i+1]
                ...
                if image_latents is not None:
                    ...
                    timesteps_next = torch.cat([
                        torch.zeros((
                            timesteps.shape[0], reference_frame_count,
                            timesteps.shape[2]
                        ), dtype=timesteps.dtype, device=self.device),
                        timesteps_next[:, reference_frame_count:]
                    ], 1)
                ...
                # test method in consistency model, cyr: hard code for linear denoise, xt = t*e + (1-t)*x0, v = e-x0
                sigmas = timesteps/1000
                sigmas_next = timesteps_next/1000
                sigmas, sigmas_next = sigmas[..., None, None, None], sigmas_next[..., None, None, None]
                latents = noise_pred * (-sigmas) + latents
                latents = latents * (1-sigmas_next) + (sigmas_next) * torch.randn_like(latents) * getattr(self.test_scheduler, "init_noise_sigma", 1)
                # latents = self.test_scheduler.step(noise_pred, t, latents)\
                #     .prev_sample
        ...
        with torch.no_grad():
            if self.is_temporal_vae:
                view_count = latents.shape[2]
                cur_latents = einops.rearrange(
                    latents, "b t v c h w -> (b v) c t h w").clone().detach()
            else:
                cur_latents = latents.flatten(0, 2).clone().detach()
            image_tensor = dwm.functional.memory_efficient_split_call(
                self.vae, cur_latents.to(dtype=self.vae.dtype),
                lambda block, tensor: block.decode(
                    tensor / block.config.scaling_factor + shift_factor,
    ...
    def inference_pipeline_record_ODE_traj(
        self, latent_shape, batch, output_type, image_latents=None,
        reference_frame_count: int = 0, start_timestep: int = 0,
        stop_timestep=None, take_time: int = 0, frozen_ref_count: int = 0,
    ):
        ...
        ## cyr, only for few-step diffsion distillation
        assert diffusion_forcing_mode is False
        ...
        if diffusion_forcing_mode and image_latents is not None:
            ...
            latents = torch\
                .randn(latent_shape, generator=self.generator).to(self.device) * \
                getattr(self.test_scheduler, "init_noise_sigma", 1)
            # 注入 reference frame 的 clean latent
            if not diffusion_forcing_mode and image_latents is not None:
                latents[:, :reference_frame_count] = image_latents[:, :reference_frame_count]
        ...
        for i in range(start_timestep, stop_timestep):
            ...
            # 恢复 reference frame 的 clean latent，避免 scheduler step 覆盖
            if not diffusion_forcing_mode and image_latents is not None:
                latents[:, :reference_frame_count] = image_latents[:, :reference_frame_count]
            ...
            timestamp_list.append(timesteps.cpu().clone())
        
        timestamp_list.append(torch.zeros_like(timestamp_list[-1]))
    ...
    @torch.no_grad()
    def few_step_ar_rollout(self, batch, latent_shape, output_type="pt"):
        """
        Few-step autoregressive rollout: 滑动窗口 + 少步去噪生成长视频
        配置来自 inference_config["few_step_ar"]

        参数说明:
        - segment_len: 每次推理的片段长度（包含 reference 帧）
        - chunk_size: 每次滚动步长，决定 reference_frame_count = segment_len - chunk_size
        - slide_step: 每轮实际前滑的帧数（默认1），生成后只保留 reference 之后的 slide_step 帧
        - selected_timesteps: 去噪步数列表（如 [1000, 750, 500, 250]）
        - max_generated_iteration: 最大连续 rollout 轮数，超过后重置为 GT
        """
        config = self.inference_config.get("few_step_ar", {})
        segment_len = config.get("segment_len", 6) ### 总长度
        chunk_size = config.get("chunk_size", 3)  ### 生成帧=总长度-reference num
        slide_step = config.get("slide_step", -1)
        selected_timesteps = config.get("selected_timesteps", [1000, 750, 500, 250])
        max_generated_iteration = config.get("max_generated_iteration", 10000)

        seq_len = batch["vae_images"].shape[1]
        batch_size, _, view_count = batch["vae_images"].shape[:3]
        reference_frame_count = segment_len - chunk_size
        generated_latents = None
        full_latents = []
        if slide_step < 0:
            slide_step = chunk_size

        for i in range(0, seq_len-reference_frame_count, slide_step):
            # 定期重置为 GT，防止误差累积
            if i % (slide_step * max_generated_iteration) == 0:
                generated_latents = None

            remaining = seq_len - i
            end = i + min(remaining, segment_len)

            if remaining >= segment_len:
                cur_segment_len = segment_len
                cur_reference_frame_count = reference_frame_count
                cur_latent_shape = latent_shape
                # 从上一轮生成帧中取最后 reference_frame_count 帧作为 reference
                cur_ref_latents = generated_latents[:, slide_step:slide_step+reference_frame_count] if generated_latents is not None else None
            else:
                cur_segment_len = remaining
                cur_reference_frame_count = min(reference_frame_count, remaining)
                cur_latent_shape = (
                    latent_shape[0], cur_segment_len,
                    *latent_shape[2:]
                )
                if generated_latents is not None:
                    cur_ref_latents = generated_latents[:, slide_step:slide_step+reference_frame_count]
                else:
                    cur_ref_latents = None

            sliced_batch = self.slice_batch(batch, i, end)

            result = self.inference_pipeline_few_step(
                cur_latent_shape, sliced_batch, output_type,
                image_latents=cur_ref_latents,
                reference_frame_count=cur_reference_frame_count,
                selected_timesteps=selected_timesteps + [0],
                is_train=False
            )

            generated_latents = result["latents"]  # (B, cur_segment_len, V, C, H, W)
            if i == 0:
                full_latents.append(generated_latents[:, :reference_frame_count])
            if remaining >= segment_len:
                # 只保留 reference 之后的 slide_step 帧，丢弃之后的生成帧
                full_latents.append(generated_latents[:, reference_frame_count:reference_frame_count + slide_step])
            else:
                new_frame_count = min(slide_step, cur_segment_len - cur_reference_frame_count)
                full_latents.append(generated_latents[:, cur_reference_frame_count:cur_reference_frame_count + new_frame_count])

        full_latents = torch.cat(full_latents, dim=1)  # (B, total_frames, V, C, H, W)

        # 解码完整视频
        images = self.decode_latents(full_latents)  # (B*T*V, C, H, W)
    ...
    def preview_pipeline(
        ...
        latent_width = batch["vae_images"].shape[-1] // \
            (2 ** (len(self.vae.config.down_block_types) - 1))
        if "few_step_ar" in self.inference_config:
            latent_shape = (
                batch_size,
                self.inference_config["few_step_ar"]["segment_len"],
                view_count, self.vae.config.latent_channels, latent_height,
                latent_width
            )
            pipeline_output = self.few_step_ar_rollout(
                batch, latent_shape, "pt")
        elif "sequence_length_per_iteration" in self.inference_config and self.inference_config["sequence_length_per_iteration"] > 0:
            latent_shape = (
                batch_size,
        ...
        if self.should_save or (
            ...
            if sequence_length == 1:
                ...
                npy_dir = os.path.join(output_path, "preview_numpy")

                if seq_label is not None:
                    video_dir = os.path.join(video_dir, seq_label)
                    txt_dir = os.path.join(txt_dir, seq_label)
                    npy_dir = os.path.join(npy_dir, seq_label)

                os.makedirs(video_dir, exist_ok=True)
                os.makedirs(txt_dir, exist_ok=True)
                os.makedirs(npy_dir, exist_ok=True)

                video_output_path = os.path.join(video_dir, "{}.mp4".format(filename))
                dwm.utils.preview.save_tensor_to_video(
                ...
                # save npy
                numpy_output_path = os.path.join(
                    npy_dir, "{}.npy".format(filename))
                preview_uint8 = (
                    preview_tensor.detach().float().clamp(0.0, 1.0)
                    .mul(255.0).round().to(torch.uint8).cpu().numpy()
                )
                preview_uint8 = np.transpose(preview_uint8, (0, 2, 3, 1))
                np.save(numpy_output_path, preview_uint8)
            ...
            # # 额外保存可视化视频为 .npy 文件 (uint8, 0-255)
            # # preview_tensor 是 float 张量，值域 [0, 1]（和 save_tensor_to_video
            # # 里走 to_pil_image 的约定一致），直接 .astype(uint8) 会截断为 0，
            # # 这里先线性映射到 [0, 255] 再转 uint8，并把通道维放到最后，方便
            # # 用 cv2/imageio/matplotlib 等按视频帧直接可视化。
            # os.makedirs(os.path.join(output_path, "preview_numpy"), exist_ok=True)
            # numpy_output_path = os.path.join(
            #     output_path, "preview_numpy", "{}.npy".format(filename))
            # preview_uint8 = (
            #     preview_tensor.detach().float().clamp(0.0, 1.0)
            #     .mul(255.0).round().to(torch.uint8).cpu().numpy()
            # )
            # if sequence_length == 1:
            #     # [C, H, W] -> [H, W, C]
            #     preview_uint8 = np.transpose(preview_uint8, (1, 2, 0))
            # else:
            #     # [T, C, H, W] -> [T, H, W, C]
            #     preview_uint8 = np.transpose(preview_uint8, (0, 2, 3, 1))
            # np.save(numpy_output_path, preview_uint8)
