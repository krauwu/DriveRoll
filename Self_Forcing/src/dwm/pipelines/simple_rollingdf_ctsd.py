...
class CrossviewTemporalSD():
    ...
    def inference_pipeline(
        ...
        img_black = (not torch.isfinite(result["images"]).all()) or \
            (result["images"].abs().max() < 1e-6) or \
            ((result["images"].max() - result["images"].min()) < 1e-6)

        if img_black:
            print("[BLACK@IMG] take_time=", take_time,
          " start_t=", start_timestep, " stop_t=", stop_timestep,
          " ref_count=", reference_frame_count,
          " latent_T=", latent_shape[1])
