import os, json, time, pickle as pkl
import numpy as np
import torch
import cv2
...
def make_image_description_string(caption_dict: dict, settings: dict, random_state=None):
    ...
    if settings.get("reorder_keys", False):
        keys = [keys[i] for i in random_state.permutation(len(keys))]

    dr = settings.get("drop_rates")
    if dr:
        keys = [k for k in keys if not (k in dr and random_state.rand() <= dr[k])]
...
class NuPlanDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # text & subkey
        text_anno='../data/annotations/nuplan_text.json',
