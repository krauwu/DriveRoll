...
class DMDWrapper:
    ...
    def train_step(self, batch: dict, global_step: int, only_fake_score: bool = False, 
        ...
        for i in range(RATIO):
            critic_pred_latent, critic_noisy_latents, critic_target = self.fake_score.train_step_critic(batch, latents.clone().detach(), global_step*RATIO+i)
            print("critic loss")
        # if only_fake_score:
        #     print("only train fake score")
        #     return {}, latents.clone().detach(), images
...
if __name__ == "__main__":
    ...
    # ### 替换dmd trained ckpt
    # config["pipeline"]["model_checkpoint_path"] = '/cache/c30078487/checkpoints/75.pth'

    # 单卡训练优化配置
    ddp = "LOCAL_RANK" in os.environ and torch.distributed.get_world_size() > 1
    if not ddp:
    ...
    if not args.only_eval:
        ...
        real_score_config = config.copy()
        # 添加标记表示这是 real_score
        # real_score_config["pipeline"]["is_real_score"] = True
        real_score_pipeline = dwm.common.create_instance_from_config(
            config["pipeline"], output_path=output_path, config=real_score_config,
            device=device, resume_from=args.resume_from)
        # 冻结 real_score 的模型参数
        for param in real_score_pipeline.model.parameters():
            param.requires_grad = False

        # fake_score: 与 generator 共享模型参数
    ...
    if args.only_eval:
        ...
        for epoch in range(config["train_epochs"]):
            ...
            for batch in training_dataloader:
                ...
                # preview: 只在 global_step % args.preview_steps == 0 时执行，且只在 rank 0 执行
