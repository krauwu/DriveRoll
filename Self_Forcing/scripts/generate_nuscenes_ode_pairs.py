...
基于 Per-step-ARDWM 项目的 diffusion model，在 nuScenes 上生成 ODE 轨迹对。
通过修改 inference_pipeline 来捕获去噪过程中的 latents。

使用方法:
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 \               
      scripts/generate_nuscenes_ode_pairs.py \                                
      --config_path configs/generate_ode/default.json \          
      --output_folder output/nuscenes_ode_pairs \ 
      --num_samples 1000
"""
...
class ODEGenerator:
    ...
    def generate_ode_pairs(self, batch, seed=None):
        ...
        # 运行推理
        capture = self.pipeline.inference_pipeline_record_ODE_traj(latent_shape, chunk_batch, "pt")
...
def main():
    ...
    for idx, batch in enumerate(tqdm(
        ...
        try:
            ...
            # find the nearest time index for each selected timestep
            ts = [t[0][0][0].item() for t in capture_times]
